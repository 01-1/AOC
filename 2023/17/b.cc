...
int run() {
  ...
  pq.e(make_pair(0, 1), make_pair(0, 0));
  pq.e(make_pair(0, 2), make_pair(0, 0));
  pq.e(make_pair(0, 3), make_pair(0, 0));

  v<char> dbij(n * m * 4 * 12);
  while (!pq.empty()) {
    auto &[hd, pos] = pq.top();
    auto [hl, db] = hd;
    auto [i, j] = pos;
    pq.pop();
    if (dbij[db * n * m + i * m + j]) {
      continue;
    }
    dbij[db * n * m + i * m + j] = 1;
    ...
    if (db / 4 >= 4) {
      ...
      }
      {
        ...
        if (0 <= di && di < n && 0 <= dj && dj < m) {
          pq.e(make_pair(hl + g[di][dj] - '0', 4 + (db % 4) ^ 1), make_pair(di, dj));
        }
      }
      {
        ...
        dj += j;
        if (0 <= di && di < n && 0 <= dj && dj < m)
          pq.e(make_pair(hl + g[di][dj] - '0', 4 + (db % 4) ^ 3), make_pair(di, dj));
      }
    }
    if (db / 4 < 10) {
      auto [di, dj] = dir[db % 4];
